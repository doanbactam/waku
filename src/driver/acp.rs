...
fn sdk_agent(
    ...
    // cwd field. Keep Windows launch direct instead of introducing cmd.exe
    // quoting and child-process lifecycle problems.
    #[cfg(windows)]
    let config = {
        let _ = cwd;
...
mod tests {
    ...
    fn sdk_agent_launches_the_binary_directly_on_windows() {
        ...
        assert_eq!(agent.config().command(), Path::new("agent.exe"));
    ...
    fn sdk_agent_uses_env_to_set_the_unix_process_cwd() {
        ...
        assert_eq!(agent.config().command(), Path::new("/usr/bin/env"));
        assert_eq!(
